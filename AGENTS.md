...
9. Returns the compiled CSS URL (passing through any query string from the original `.scss` URL), with `h=<hash>` appended unless `sassy-version-url` returns false. The hash is the first twelve of the build's md5, recorded at the compile as `__hash__` so a cached request reads it from the record, and the map is written with the same value as `x_sassy_css`. `ver` only moves when a site bumps it, and a CDN held a build from before a push under an unchanging URL while the map was never cached: Capture aligned one build's text to another's map and located nothing. The JS compares the `h` of the URL it fetched the text from with the map's `x_sassy_css` and withholds lines on a mismatch, **and a map with no stamp is a mismatch**: the first version skipped the check when the stamp was absent, and a browser that had kept an unstamped map on heuristic freshness (the map is served with no `Cache-Control`, so Chrome keeps it for a tenth of its age) was aligned anyway. The map is fetched `no-cache`, then once `no-store` if it still disagrees, from the address the sheet itself names: `Printer::version_map_link()` rewrites the `sourceMappingURL` to `…css.map?v=<token>`, a new address per build, which is also what stops DevTools mapping a new sheet against a cached old map. The token is the md5 of the CSS as it reached that step, since the CSS cannot carry its own hash. A located sheet whose change has no map segment says so on that change's line.
...
|---|---|---|
| `sassy-filemtimes-{handle}` | `build_file => filemtime`, `__compile_time__`, `__diagnostics__` (severity tally), `__hash__` (the build's md5 prefix, for the served URL), `deps`, `dirs`, `truncated` | Directory creation error; rewritten after each successful compile |
...
| Output built by a recorded handle this process did not register | `notice` | never |
...
**Orphan detection is scoped.** Dotfiles and directories are skipped, because the Dart engine's own `.sassy-tmp` lives in the build directory and reporting Sassy's own working directory on a first run is how a tool teaches people to ignore it. It stays a warning however well it is scoped: a handle enqueued only on some template is discovered by no hook set at all, and its output is indistinguishable from an abandoned one. What the cache does know is who built a file: `Compile_Cache::builder_of($path)` walks the handle index (so it works under an external object cache, where the options table cannot be enumerated) for a record keyed by that build path. An unclaimed output with a builder on record is a notice naming the handle and its last compile (the build file's mtime as the record keys it; `__compile_time__` is the compile's duration in seconds, which the first version of this notice formatted as a date and printed 1970), since a plugin whose bootstrap returns early under `WP_CLI` registers nothing here while its web compiles left exactly that record. A first version counted contexts that registered nothing, and the install that motivated it had 25 admin styles from other plugins, so that unit was wrong.
...
**Cache-busting uses the build's content hash**, `meta.hash`, which is exact where `Math.random()` was merely different and is the primitive an opt-in change poll would compare. A reload sets it as `h`, the same parameter the server writes, and a reload with no hash known drops `h` rather than let it lie. Every client-side swap also carries `r`, the page's swap count, through the one `swapUrl()` all three reload paths use: a content-keyed URL means identical source is an identical address, so delete, Push, re-add, Push returns to the first build's URL, and Chrome's inspector, which keeps edited stylesheet text per URL for the page's life, drops every source range of a sheet loaded again at a URL it edited and shows its rules read-only. The server's URL stays content-keyed for visitors and CDNs; only the developer's own swaps are unique. The endpoint's `href` is the new build's URL and the page's link is the old build's, so the two swap sites find the link with `sameFile()`, host and path, never by substring: matching the full URL found the link only when nothing had changed, which silently stopped Live Compile, the poll, the meta log and the post-push re-baseline for one commit. **Logging → Auto-reload** polls the compile endpoint every two seconds with the page's context and swaps only the sheets whose hash moved; off by default, per browser, paused while the tab is hidden, and it stops itself when the endpoint stops answering cleanly. The stack summary toggle waits for phase 8.
...
| `sassy-version-url` | `true` | Whether the served URL carries `h=<build hash>`. Off, a CDN can pair a stale sheet with a fresh map and Capture can only guess |
