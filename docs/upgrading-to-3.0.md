...
The URL `style_loader_src` returns now ends in `&h=` and twelve hex characters, the build's md5 prefix. Anything that matched the compiled URL exactly, a preload header, a critical-CSS tool, a test, should match up to the query string or bind `sassy-version-url` to false. The map gains one key, `x_sassy_css`, which tools ignore, and the CSS's `sourceMappingURL` ends in `?v=` and twelve hex characters. The compile endpoint's `href`, and so `sassy:compiled`, carries the new build's `h` too: a listener that finds the page's link by comparing URLs should compare host and path, as Sassy's own swap now does, because the page's link carries the old `h` exactly when there is something to swap.
